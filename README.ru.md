...
Для уже одобренной локальной настройки мейнтейнера тот же явный флаг `--insecure-tls` можно добавить к `completion` или `send-file`, если отправка завершается ошибкой `CERTIFICATE_VERIFY_FAILED`. Для новых установок этот режим не является настройкой по умолчанию.
