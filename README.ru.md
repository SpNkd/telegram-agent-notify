...
2. Добавьте бота в нужную группу (для темы — в саму группу).
3. Запустите `telegram-notify configure` и вставьте токен в скрытое приглашение.
4. CLI покажет одноразовую команду вида `/start@имя_бота TN-ABCD2345`.
5. Отправьте эту команду в нужное место: в личный чат с ботом либо внутри целевой группы/темы.
6. Нажмите Enter в терминале. Утилита сама получит `chat_id` и, если это тема, `message_thread_id`.
7. Подтвердите параметры и отправьте тестовое уведомление.

Для темы группы команду нужно отправить именно внутри этой темы. Команда `/start` специально используется потому, что Telegram передаёт команды боту даже при включённом privacy mode. Бот всё равно должен быть участником группы.

Одноразовый код действует только для текущего запуска `configure`; идентификаторы искать вручную не нужно.
...
Для уже одобренной локальной настройки мейнтейнера каждый вызов `completion` и явно запрошенный `send-file` должен сразу содержать явный флаг `--insecure-tls`, а не добавлять его только после ошибки сертификата. Для новых установок этот режим не является настройкой по умолчанию.

Если sandbox агента сообщает `nodename nor servname provided`, `Name or service not known` или другую DNS-ошибку для `api.telegram.org`, сбой произошёл до обращения к Telegram. Повторите ту же команду один раз через системный/host network-доступ, который предоставляет окружение агента. `--insecure-tls` не влияет на DNS; для одобренной локальной настройки сохраните этот флаг при повторе. Если host network тоже недоступен, прекратите повторы и сообщите об ошибке.
